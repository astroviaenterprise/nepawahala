  ...
  return (
    <div className={cn("relative", className)}>
      <div className="relative group">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-amber-500 w-4 h-4 transition-colors" />
        <input
          type="text"
          value={inputValue}
          onChange={handleInputChange}
          placeholder={placeholder || "SCAN NETWORK FOR NODE..."}
          className="w-full pl-12 pr-4 py-3.5 rounded-xl border border-zinc-700 bg-zinc-900 text-white placeholder:text-zinc-500 focus:ring-1 focus:ring-amber-500/50 focus:border-amber-500 outline-none transition-all shadow-inner font-mono text-xs uppercase tracking-tight"
        />
      </div>
            ...
            <>
              {suggestions.map((s) => (
                ...
                >
                  ...
                  <div>
                    <div className="text-[11px] font-bold text-white uppercase tracking-tight">{s.structured_formatting.main_text}</div>
                    <div className="text-[9px] text-zinc-400 font-mono uppercase tracking-tighter">{s.structured_formatting.secondary_text}</div>
                  </div>
                </button>
              ))}
              {inputValue && (
                ...
                >
                  ...
                  <div className="text-[11px] font-bold text-zinc-200 uppercase tracking-tight">Use Manual Entry</div>
                  <div className="text-[9px] text-zinc-400 font-mono ml-auto">"{inputValue}"</div>
                </button>
              )}
            </>
          )}
